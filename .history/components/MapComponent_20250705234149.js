// components/MapComponent.js - Complete version with basemap support
...
// Komponen untuk mengganti TileLayer ketika basemap berubah
const BasemapLayer = ({ style }) => {
  ...
  const basemapConfigs = {
    ...
    outdoor: {
      ...
      attribution: '&copy; <a href="https://www.cyclosm.org">CyclOSM</a> & <a href="http://osm.org/copyright">OpenStreetMap</a> contributors',
      maxZoom: 20
    },
    dark: {
      url: 'https://tiles.stadiamaps.com/tiles/alidade_smooth_dark/{z}/{x}/{y}{r}.png',
      attribution: '&copy; <a href="https://stadiamaps.com/">Stadia Maps</a>, &copy; <a href="https://openmaptiles.org/">OpenMapTiles</a> &copy; <a href="http://osm.org/copyright">OpenStreetMap</a> contributors',
      maxZoom: 20
    }
  };

  const config = basemapConfigs[style] || basemapConfigs.streets;

  useEffect(() => {
    // Force map to update tiles when style changes
    map.invalidateSize();
  }, [style, map]);
...
// Main MapComponent with enhanced real-time GPS positioning and basemap support
const MapComponent = forwardRef(({ 
  ...
  // NEW: Props for sidebar alignment
  sidebarWidth = 0,
  ...
  // NEW: Handle basemap style change
  const handleBasemapStyleChange = (newStyle) => {
  ...
  return (
    <>
      <MapContainer
        ...
        zoomControl={false}
      >
        {/* UPDATED: Use BasemapLayer component for dynamic tile switching */}
      ...
      {/* NEW: Basemap Selector Component - Only show when not in drawing mode */}
      {!isDrawingMode && (
        <BasemapSelector
          currentStyle={basemapStyle}
          onStyleChange={handleBasemapStyleChange}
...
export default MapComponent;-700">{vehicle.position.lat.toFixed(4)}</span>
                            </div>
                            <div>
