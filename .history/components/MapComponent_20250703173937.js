...
const DrawingHandler = forwardRef((props, ref) => {
  const { isDrawingMode, drawingType, onPolygonComplete, onCircleComplete } = props;
  const map = useMap();
    ...
    click: (e) => {
      ...
      if (drawingType === "polygon") {
        ...
        if (!isDrawing) {
          ...
          if (radius >= 10) {
            // Gunakan onCircleComplete untuk circle, bukan onPolygonComplete
            onCircleComplete({
              center: circleCenter,
              radius: radius
            });
...
const MapComponent = forwardRef(({ 
  ...
  drawingType = "polygon",
  onPolygonComplete,
  onCircleComplete,  // Tambahkan parameter ini
  geofences = [],
  allGeofences = [],
  onGeofenceDeleted,
