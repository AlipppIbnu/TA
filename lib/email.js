// lib/email.js - Email service dengan connection pooling dan multiple templates
...
export const verifyEmailTransporter = async () => {
  try {
    ...
    transporter = null; // Reset transporter jika verify gagal
    throw error;
  }
};

// Template untuk Reset Password OTP
export const sendResetPasswordOTP = async (email, otp) => {
  ...
  const mailOptions = {
    ...
    `,
    text: `Kode OTP Reset Password Anda adalah: ${otp}. Kode ini akan kedaluwarsa dalam 1 menit.`
  };

  return transport.sendMail(mailOptions);
};

// Template untuk Device Verification OTP
export const sendDeviceVerificationOTP = async (email, otp) => {
  const transport = createTransporter();
  ...
  const mailOptions = {
    ...
    `,
    text: `Kode OTP Verifikasi Perangkat Anda adalah: ${otp}. Kode ini akan kedaluwarsa dalam 1 menit.`
  };

  return transport.sendMail(mailOptions);
};

// Generic send email function dengan timeout
export const sendEmailWithTimeout = async (mailOptions, timeoutMs = 10000) => {
  const transport = createTransporter();
...
export const sendOTPEmail = sendResetPasswordOTP;